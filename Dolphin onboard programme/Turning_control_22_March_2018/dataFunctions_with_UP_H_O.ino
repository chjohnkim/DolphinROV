...
//  Serial.print(xPropOutput);
  
//  Serial.print(" D output: ");
//  Serial.print(xDerivativeOutput);
...
void pidControl()
{
  ...
  xDerivativeOutput = -xDerivativeGain * gyro.x();
  yDerivativeOutput = -yDerivativeGain * gyro.y();
  zDerivativeOutput = -zDerivativeGain * gyro.z();
...
//  eulZPrev = eulZ;
  ...
  {
    xOutput = xPropOutput + xDerivativeOutput;
  }

  if (eulY > 2 | eulY < -2) // Deadband
  {
    yOutput = yPropOutput + yDerivativeOutput;
  }

  if (eulZ > 2 | eulZ < -2) // Deadband
  {
    zOutput = zPropOutput + zDerivativeGain;
  }
}
...
void feedMotorPulse() {
  ...
  feedMotor1 = motorStop1 - yOutput + xOutput + thrust_input;// - zOutput, Top Motor ( I facing the tail )  // ADJUST THIS FOR QUADROTOR DYNAMICS ADUSTMENT
  ...
  feedMotor2 = motorStop2 - yOutput - zOutput - thrust_input - Forward_thrust;//- xOutput , Right Motor ( I facing the tail )
  ...
  feedMotor3 = motorStop3 - yOutput - xOutput + thrust_input; //+ zOutput; , Bottom Motor ( I facing the tail )
  ...
  feedMotor4 = motorStop4 - yOutput  + zOutput - thrust_input - Forward_thrust;//+ xOutput , Left Motor ( I facing the tail )
...
if(Turning_con_switch == 1){
  
    desired_yaw = (6.283 / (1000*Turning_gain))*(millis()-Start_Time);    // Turning control 

}
    toQuaternion( desired_roll , desired_pitch, desired_yaw);

   // qRef1 = Depth_con_qRef1 - (Turning_gain / 10000)*(millis()-Start_Time);
    qRef1 = q[1];
    qRef2 = q[2];
    qRef3 = q[3];
    qRef4 = q[4];
   // qRef4 = Depth_con_qRef4 + (Turning_gain / 10000)*(millis()-Start_Time);
 ...
 while (true) {
  ...
  if(prefix=='D')
  {
    ...
    if((Desire_depth - Desire_depth_range)<Current_depth<(Desire_depth + Desire_depth_range))
    {
   ...
   if((Desire_depth - Desire_depth_range)>Current_depth)
    {
      if(T1 == 1){
     // qRef2 = Depth_con_qRef2 + Depth_con_tilt_up;
     
    //   qRef2 = (Desire_depth - Current_depth)*Depth_con_tilt_gain;

    desired_pitch = (Desire_depth - Current_depth)*Depth_con_tilt_gain;
    ...
    qRef2 = q[2];
        ...
        if(desired_pitch > Depth_con_tilt_max){
          
          desired_pitch = Depth_con_tilt_max;
    ...
    if((Desire_depth + Desire_depth_range)<Current_depth)
    {
     ...
     if(T1 == 1){
     // qRef2 = Depth_con_qRef2 + Depth_con_tilt_down;
     
       // qRef2 = (Desire_depth - Current_depth)*Depth_con_tilt_gain;
       desired_pitch = 6.283 + (Desire_depth - Current_depth)*Depth_con_tilt_gain;

    toQuaternion( desired_roll , desired_pitch, desired_yaw);

    qRef2 = q[2];
    
       if(desired_pitch < 6.283 -Depth_con_tilt_max){
          
          desired_pitch = 6.283 -Depth_con_tilt_max;
...
if(sensor.getPressure(ADC_4096)<30000 && sensor.getPressure(ADC_4096)>-50){
  Current_depth = sensor.getPressure(ADC_4096) - 1000.0;
}

}

void print_parameter_setting()

{

  Serial.print("xPropGain = ");
       Serial.println(xPropGain);
       Serial.print("yPropGain = ");
       Serial.println(yPropGain);
       Serial.print("zPropGain = ");
       Serial.println(zPropGain);

       Serial.print("xDerivativeGain = ");
       Serial.println(xDerivativeGain);
       Serial.print("yDerivativeGain = ");
       Serial.println(yDerivativeGain);
       Serial.print("zDerivativeGain = ");
       Serial.println(zDerivativeGain);

       Serial.print("Depth_con_qRef1 = ");
       Serial.println(Depth_con_qRef1);
       Serial.print("Depth_con_qRef2 = ");
       Serial.println(Depth_con_qRef2);
       Serial.print("Depth_con_qRef3 = ");
       Serial.println(Depth_con_qRef3);
       Serial.print("Depth_con_qRef4 = ");
       Serial.println(Depth_con_qRef4);

       Serial.print("Depth_con_tilt_up = ");
       Serial.println(Depth_con_tilt_up);
       Serial.print("Depth_con_tilt_down = ");
       Serial.println(Depth_con_tilt_down);
       Serial.print("Depth_con_tilt_max = ");
       Serial.println(Depth_con_tilt_max);
       Serial.print("Depth_con_tilt_gain = ");
       Serial.println(Depth_con_tilt_gain);

       Serial.print("Thrust gain = ");
       Serial.println(Thrust_gain);
       Serial.print("Depth_thrust_gain = ");
       Serial.println(Depth_thrust_gain);

       Serial.print("Depth_recv_gain = ");
       Serial.println(Depth_recv_gain);

       Serial.print("Desire depth_range = ");
       Serial.print(Desire_depth_range);
       Serial.println("mm");
       ...
       Serial.print(Exper_Duration);
       Serial.println(" sec");

   

       Serial.print("Turning gain = ");
       Serial.print(Turning_gain);
       Serial.println(" sec/rev ");

       //    Serial.print("M_F_T = ");
       Serial.println(Max_Forward_thrust);

       Serial.print("Accele Max Time = ");
       Serial.println(Accele_Max_Time);
  //     Serial.println(" sec ");


   //    Serial.print("Turning switch = ");
   //    Serial.println(Turning_con_switch);
