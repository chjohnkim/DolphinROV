...
double xPropGain = 3.0;   // Pitch // Proportional Gain constant
double yPropGain = 3.0;   // Roll
double zPropGain = 1.0;   // Yaw
...
double xDerivativeGain = 100.0; // Derivative Gain Constant
double yDerivativeGain = 100.0;
double zDerivativeGain = 50.0;
...
float Depth_con_qRef2 = 0.0;     //Pitch -ve = tilt up
float Depth_con_qRef3 = 0.0;
float Depth_con_qRef4 = 0.0;

double desired_roll = 0.0 * (6.283/360) ;  //unit : rad
double desired_pitch = 0.0 * (6.283/360) ;  //unit : rad
double desired_yaw = 0.0 * (6.283/360) ;  //unit : rad
float q[4] = {0,0,0,0};

float Depth_con_tilt_up = 20.0 * (6.283/360) ;  //unit : rad   
float Depth_con_tilt_down = -20.0 * (6.283/360) ;  //unit : rad
float Depth_con_tilt_max = 20.0 * (6.283/360) ;  //unit : rad
float Depth_con_tilt_gain = 0.1;
float Thrust_gain = 0.5;
float Depth_thrust_gain = 3.0 ;
float Depth_recv_gain = 1.0;
float Desire_depth = 1000.0;
float Desire_depth_range = 0.5; // unit : mm
float Current_depth = 1000.0;  // unit : mm
float Set_Desire_depth = 50.0; //unit : cm
float Forward_thrust = 0;
float Max_Forward_thrust = 40;

//Speed Ramping

float Accele_run_Time = 0;
float Accele_Max_Time = 120;  // unit : sec


//Turning

float Turning_gain = 10; // unit : sec / rev
float Turning_con_switch = 1 ; // 1: ON , 0: OFF

//Set Experiment duration
unsigned long Start_Time = 0;
unsigned long End_Time = 0;
unsigned long Exper_Duration = 240; // Unit : second
unsigned long Exper_Duration_milli = 0; // Unit : millisecond
unsigned long Experiment_clock = 0 ;
...
int s1 = 0; // temporary variable to stop the motor
...
*/
  ...
  motor4.write(motorStop1); 
      ...
      case 's':
        ...
        print_parameter_setting();
        
        
       Serial.println("Depth control commence");

       Start_Time = millis();
     Exper_Duration_milli = Exper_Duration * 1000;
     End_Time = Start_Time + Exper_Duration_milli;
...
//       T2 = 0;
//
//       Serial.println("Orientation Control Mode");
//
//       break;
//
       case '9':
       ...
       Serial.println("Depth Control Mode");
...
/////////////////////////////////////////////Ramping code
         Accele_run_Time = millis();

         if(Accele_run_Time > (Accele_Max_Time*1000)){

          Accele_run_Time = Accele_Max_Time*1000;
          
         }

        Forward_thrust = (Accele_run_Time/(Accele_Max_Time*1000))*Max_Forward_thrust;
